...
6. Is nblessed efficient?
  - Yes. nblessed implements CSR and uses the painter's algorithm to render the
    screen. It maintains two screen buffers so it only needs to render what
    has changed on the terminal screen.
7. Will nblessed work with all terminals?
  - Yes. nblessed has a terminfo/termcap parser and compiler that was written
...
8. What is "curses" and "ncurses"?
  - ["curses"][curses] was an old library written in the early days of unix
    ...
    programs. nblessed uses neither of these, and instead handles terminal
    compatibility itself.
9. What is the difference between nblessed and nblessed-contrib?
  - nblessed is a major piece of code which reimplements curses from the ground
    up. A UI API is then layered on top of this. [nblessed-contrib][contrib] is
    a popular library built on top of nblessed which makes clever use of modules
    to implement useful widgets like graphs, ascii art, and so on.
10. Are there nblessed-like solutions for non-javascript platforms?
