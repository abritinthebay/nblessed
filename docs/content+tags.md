...
nblessed tags support the basic 16 colors for colors, as well as up to 256
colors.
...
Escaping can either be done using `nblessed.escape()`

```
box.setContent('here is an escaped tag: ' + nblessed.escape('{bold}{/bold}'));
```
