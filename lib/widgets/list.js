...
"underline",
		"blink",
"inverse",
"invisible"].forEach(function(name) {
	...
	if (options.items) {
		this.ritems = options.items.slice(0);
		// Operate on a clone so we don't accidentally mutate the data
		options.items.slice(0).forEach(this.add.bind(this));
	}
	...
	if (options.keys) {
         ...
         */
				var visible = Math.min(self.height - self.iheight,
...
List.prototype.createItem = function(content) {
	var self = this;
	const rowStyle = this.rowStyles[this.items.length] || {};
	// Note: Could potentially use Button here.
	var options = {
		screen: this.screen,
		content: content,
		align: rowStyle.align || this.align || "left",
		top: 0,
		left: 0,
		right: this.scrollbar ? 1 : 0,
		tags: this.parseTags,
		height: 1,
		hoverEffects: this.mouse ? rowStyle.hover || this.style.item.hover : null,
		focusEffects: this.mouse ? rowStyle.focus || this.style.item.focus : null,
		autoFocus: false
	};
