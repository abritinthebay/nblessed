/*
 * Basic curses-like functionality for blessed.
 */
...
Program.prototype._ncoords = function() {
	if (this.xpos < 0) {
		this.xpos = 0;
	} else if (this.xpos >= this.cols) {
		this.xpos = this.cols - 1;
	}
	if (this.ypos < 0) {
		this.ypos = 0;
	} else if (this.ypos >= this.rows) {
		this.ypos = this.rows - 1;
	}
};
...
Program.prototype.omove = function(x, y) {
	...
	if (y === this.ypos && x === this.xpos) {
		return;
	}
	if (y === this.ypos) {
		if (x > this.xpos) {
			this.cuf(x - this.xpos);
		} else if (x < this.xpos) {
			this.cub(this.xpos - x);
		}
	} else if (x === this.xpos) {
		if (y > this.ypos) {
			this.cud(y - this.ypos);
		} else if (y < this.ypos) {
			this.cuu(this.ypos - y);
		}
	} else {
		if (!this.zero) {
			x++, y++;
...
Program.prototype.feed = function() {
	if (this.tput && this.tput.bools.eat_newline_glitch && this.xpos >= this.cols) {
		return;
	}
	this.xpos = 0;
	this.ypos++;
...
Program.prototype.saveCursor = function(key) {
	...
	this.savedX = this.xpos || 0;
	this.savedY = this.ypos || 0;
	if (this.tput) {
		return this.put.sc();
	}
...
Program.prototype.restoreCursor = function(key, hide) {
	...
	this.xpos = this.savedX || 0;
	this.ypos = this.savedY || 0;
	if (this.tput) {
		return this.put.rc();
	}
...
Program.prototype.lsaveCursor = function(key) {
	...
	this._saved[key].x = this.xpos;
	this._saved[key].y = this.ypos;
...
Program.prototype.saveCursorA = function() {
	this.savedX = this.xpos;
	this.savedY = this.ypos;
	if (this.tput) {
		return this.put.sc();
	}
