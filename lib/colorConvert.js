/* eslint id-length: 0, no-confusing-arrow: 0, no-bitwise: 0 */
const {sqrt, pow, cos, atan2, sin, abs, exp, round, min, PI} = Math;
...
const deg = (r) => r * 180 / PI; // Radians to degrees
const rad = (d) => d * PI / 180; // Degrees to radians
...
const getLCHdeltasfromLab = ([L1, a1, b1], [L2, a2, b2]) => {
	...
	const RC = 2 * sqrt(pow(Cpm, 7) / (pow(Cpm, 7) + p257));
...
const pivotLab = ([l, a, b]) => {
	...
	const fz3 = pow(fz, 3);
	return [
		fx3 > PIV_INV_EPSILON ? fx3 : (116 * fx - 16) / KAPPA,
...
const xyz2lab = (xyz) => {
	...
	const [fx, fy, fz] = xyz.map(pivotXYZ);
 ...
 */
const rgb2lab = (rgb) => xyz2lab(rgb2xyz(rgb));
...
const hex2rgb = (input) => {
	// Check if it's a short hex string (ie "#FFF") and convert to long
